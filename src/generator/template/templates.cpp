...
int renderClashScript(YAML::Node &base_rule, std::vector<RulesetContent> &ruleset_content_array, const std::string &remote_path_prefix, bool script, bool overwrite_original_rules, bool clash_classical_ruleset)
{
    ...
    for(std::string &x : groups)
    {
        ...
        if(group_has_domain)
        {
            ...
            base_rule["rule-providers"][yaml_key]["path"] = "./providers/" + std::to_string(hash_(url)).substr(0, 4) + "_" + urlDecode(yaml_key) + "_ddomain.yaml";
            if(interval)
        ...
        if(group_has_ipcidr)
        {
            ...
            base_rule["rule-providers"][yaml_key]["path"] = "./providers/" + std::to_string(hash_(url)).substr(0, 4) + "_" + urlDecode(yaml_key) + "_iipcidr.yaml";
            if(interval)
                base_rule["rule-providers"][yaml_key]["interval"] = interval;
        }
        if(!group_has_domain && !group_has_ipcidr)
        {
            ...
            base_rule["rule-providers"][yaml_key]["path"] = "./providers/" + std::to_string(hash_(url)).substr(0, 4) + "_" + urlDecode(yaml_key) + "_classical.yaml";
            if(interval)
                base_rule["rule-providers"][yaml_key]["interval"] = interval;
        }
        if(script)
